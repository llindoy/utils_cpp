...
struct factory_info_type
{
    ...
    using create_function_obj = std::shared_ptr<IB>(*)(const typename IOWRAPPER::input_object&);
...
public:
    ...
    static bool is_loadable(const typename IOWRAPPER::input_object& obj)  
    {
        ...
        {
            std::string s;
            CALL_AND_HANDLE(IOWRAPPER::load<std::string>(obj, INTERFACE_TYPE::key().c_str(), s), "Failed to load spectral density type string.");
    ...
    static std::shared_ptr<INTERFACE_TYPE> create(const typename IOWRAPPER::input_object& obj, Args&& ... args)  
    {
        try
        {
            std::string s;
